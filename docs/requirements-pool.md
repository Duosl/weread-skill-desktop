...
建议下一个启动：`REQ-007 HTML 阅读报告生成器` 的高级报告 CLI 接入。

原因：基础 HTML 报告和导出模板入口已经有进展；下一步应接入本地 CLI 封装库，把高级模板从规划中变成可执行能力。
...
- 当前进展：
  ...
  - 已将阅读报告页重构为模板目录：基础模板卡片点击后进入接近全屏的报告工作台，集中展示预览、浏览器打开、导出和分享入口占位。
  - 已在 `mvp-design-doc.md` 增补高级报告完整方案：模板包、提示词模板、输入输出目录、job 目录、CLI 调用边界、Tauri 命令和分享版 HTML 策略。
  - 已在 `AGENTS.md` 记录当前开发偏好：普通 Markdown 导出页保持现有工作台结构；阅读报告页使用模板目录和全屏报告工作台；高级模板不伪造能力；分享版与正式导出分离。
- 技术边界：
  ...
  - 高级模板目录约定为 `reports/templates/<template-id>/`，任务目录约定为 `reports/jobs/<job-id>/`，应用通过模板清单和 job 状态查看输入输出，不让前端直接操作任意路径。
  - 分享版 HTML 从 job 输出单独生成，可带应用署名、回链或安装入口；默认不包含用户未确认的原文摘录。
- 暂不做：
  - 第一版不做完整 HTML 编辑器。
  - 第一版不做在线分享平台。
