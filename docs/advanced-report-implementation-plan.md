...
- 输出读取会做基础质量校验：分析版是否过短、是否使用“你”作为主语、是否残留“这个用户/该用户”、是否包含 `书迹` 标识。
...
原文权限策略不应只用“是否必须授权”一个布尔值长期承载。后续模板元数据应使用三档策略：

| 策略 | 行为 |
|------|------|
| `required` | 模板必须依赖划线原文或个人想法作为核心证据；未授权时后端拒绝创建 job。 |
| `optional` | 模板可基于统计、书架、笔记数量和分类等低敏数据降级生成；授权后才预取 `notes.raw.json`，报告质量更具体。 |
| `none` | 模板不读取原文划线和个人想法，不需要展示原文授权为必需项。 |

无论策略如何，只要读取原文划线、书摘或个人想法，都必须先获得用户确认。用户自定义要求不能覆盖该策略；未授权的降级 job 必须在 brief 中告知 Agent 不得编造划线、想法、读后感或书籍原文。
...
- 不依赖外部网络资源。
- 不包含远程 JS。
- 不暴露 API Key、缓存路径、用户本地绝对路径或 `file://` URL。
- 不使用 iframe、object、embed、fetch、XMLHttpRequest、window.open 或 location 跳转读取/加载本地 HTML、JSON、图片或其他文件；报告必须是自包含单文件，避免浏览器 `file:` 唯一安全源限制。
