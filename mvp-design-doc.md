...
| P0 | 书架概览 | 书架列表、全部/已读完筛选、类别筛选、最近阅读时间 | `shelf_sync` |
...
> 注意：`/shelf/sync` 不提供精确阅读进度。MVP 只能稳定展示 `finishReading`、`category`、最近阅读时间等字段。书架页状态筛选只保留「全部」和「已读完」，并基于 `category` 的一级分类提供本地类别筛选；书籍卡片显示完整 `category`，只在 `finishReading=1` 时显示「读完」标签，其他书籍不显示状态标签。若要展示 0-100% 阅读进度，需要额外引入 `/book/getprogress`，不属于 P0。
...
| | `save_telemetry_enabled` | 开启 / 关闭匿名版本统计 |
| | `reset_telemetry_installation_id` | 重置随机安装编号 |
| | `send_telemetry_ping` | 启动时发送匿名版本统计 |
| **书架** | `sync_shelf` | 同步书架 |
...
│          │  [全部类别] [文学] [历史] [商业] ...        │  │
│          │  ├─────────────────────────────────────┤  │
│          │  原则        瑞·达利欧    商业  读完 2026-05-12│
│          │  三体        刘慈欣      科幻       2026-05-10│
│          │  深度工作    卡尔·纽波特  效率       -         │
│          │  ...                                    │
│          │  └─────────────────────────────────────┘  │
└─────────────────────────────────────────────────────┘
...
笔记工作台内部使用 `浏览 / 导出` 两个 Tab，Tab 紧跟页面标题展示。浏览区保留单本笔记查看体验，并把「导出当前书 / 微信读书」放在页面头部操作区；导出区保留批量选择、预览和生成文件能力。
...
智能体模版不得直接把全量笔记无节制发送给模型。必须先做数据裁剪和用户确认：只发送统计摘要、代表性书籍、代表性划线 / 想法，并明确是否包含个人笔记内容。
...
- `bookmark_list` / `my_reviews`：基础报告只使用划线数、想法数、笔记密度等统计维度；原文内容只用于笔记浏览、Markdown 导出和用户授权后的智能体报告。
...
- 基础报告不展示原划线和个人想法；报告页只呈现统计维度、趋势、排行和规则化结论。
...
- 智能体报告默认自动使用本地已安装 CLI，并沿用用户 CLI 自身的默认模型配置；当前版本不在前端暴露 Agent 或模型选择。
...
智能体报告按“模板包 + 数据准备 + 本地 CLI 任务 + 输出登记 + 应用打开/导出”实现。CLI 的具体封装库和调用参数后续再接入；本阶段先固定应用侧目录、数据契约和模板发现方式。

下一阶段的具体落地方案以 `docs/advanced-report-implementation-plan.md` 为准。该文档补充了 `agent-cli-bridge` 的真实接入方式、文件式渐进披露、无限数据预取、job 工作区、前端 Hook 与分阶段验收。
...
- `requiresRawNotesConsent`：是否必须获得原始划线 / 想法授权。当前阶段保持两类策略：必需授权模板未授权不能生成；非必需授权模板默认可不使用原文生成，用户主动勾选后才读取划线原文和个人想法。不引入 `rawNotesPolicy` 三档字段。
...
```json
{
  ...
  "outputs": ["insights.json", "report.html"]
}
```
...
- 原文划线、书摘和个人想法属于高敏数据；只要读取或写入 job 工作区，必须先由用户显式确认。
- 当前不拆分 optional / none，只区分“必需授权”和“可选授权”：必需授权模板未授权时阻断生成；可选授权模板未授权时可只使用统计、书架、笔记数量和分类等低敏数据生成。
- 未授权生成时，job 输入必须明确告知 Agent 当前没有原文授权，并要求数据不足时不要编造用户划线、想法、读后感或书籍原文。
...
| 命令 | 说明 |
|------|------|
| `list_report_templates` | 扫描内置与用户模板目录，返回基础/智能体模板清单 |
| `prepare_report_job` | 创建 job，写入 `report-data.json`、`excerpts.json`、展开后的 `prompt.md` |
| `run_advanced_report` | 调用本地 CLI，生成 `insights.json` 和 `report.html` |
...
- 当前版本不支持分享网页，不要求智能体生成 `share.html`，前端不展示分享按钮。
- 分享能力后续单独设计，不进入本版输出契约。

#### 不做内容

- 第一版不做完整网页编辑器。
- 第一版不做在线分享平台。
- 第一版不做 PDF 渲染；当前 MVP 已支持 Markdown 导出，PDF 暂不排期。
...
- 匿名版本统计：支持应用名称、随机安装编号、版本、平台、架构、语言区域和服务端 IP 的轻量统计；设置页仅在关于区域做轻量说明。
...
以下质量收敛已完成，详细完成记录归档在 `docs/archive/completed-requirements.md`：
...
当前 MVP 已完成，功能边界按以下方式冻结：

- Markdown 是当前唯一正式导出格式，PDF 不进入当前排期。
- 腾讯 ima 联动已完成当前可用闭环，但属于外部集成增强，不作为 MVP 主线继续扩展；后续根据用户反馈优化重导出逻辑、同步结果解释和导出内容文案。
- 阅读报告保留为增值能力，收尾重点是隐私授权、真实数据视觉回归和产物质量提示，不继续扩大为在线分享平台或完整报告编辑器。
- 书城搜索、推荐发现、公共书评浏览、相似书推荐、在线阅读体验继续保持在 MVP 外。

### 10.4 Post-MVP 产品方向

后续活跃需求以 `docs/current-context.md` 和 `docs/requirements-pool.md` 为入口。功能按“导出可信度 > 内容可读性 > 报告复用 > 外部集成”的顺序评估，不按完整微信读书客户端补齐能力。

| 方向 | 建议优先级 | 产品机会 | 边界 |
|------|------------|----------|------|
| 划线颜色筛选 | 已完成 | Notes 页可按划线颜色筛选，并在划线卡片显示红 / 紫 / 蓝 / 绿 / 黄颜色标记 | 当前只展示和筛选已有标注，不做划线编辑；`style` 线型字段先由后端兼容 |
| 智能体模板原文权限 | P1 | 保持必需授权 / 可选授权两类策略，把未授权时的数据边界和生成说明讲清楚 | 用户未授权时不能读取划线原文和个人想法 |
| ima 重同步体验 | P2 | 优化重复同步、跳过、失败恢复和导入后文案 | 不默认覆盖或追加用户已整理的 ima 笔记 |
| 报告真实数据回归 | P2 | 检查长书名、排行密度、小窗口、PPT / 小红书形态质量 | 不新增在线托管和分享平台 |
| 资料库索引说明 | P2 | 补充 Obsidian / 本地文件夹 / ima 的使用说明和字段解释 | 不为单个平台牺牲通用 Markdown 可读性 |
| 匿名安装规模统计 | 已完成 | 了解安装量、版本分布和平台分布，辅助发布判断 | 统计端点支持多应用 `appName`；不采集操作事件、API Key、微信读书内容、导出路径或文件名；IP 只由服务端从请求中记录 |
