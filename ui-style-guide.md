...
```
视觉重点：报告模板目录。页面本体只负责选择报告模板，具体预览和导出进入接近全屏的报告工作台

页面结构：
├── 时间范围切换：本月 / 今年 / 全部
├── 基础模板：阅读分析报告 / 读书旅程 / 年度阅读报告
├── 高级模板：阅读人格、知识盲区、下一阶段建议等规划中卡片
└── 高级模板暂未接入时可以展示规划中，但不能伪造可执行能力

报告工作台弹窗：
├── 顶部：模板名、类型、关闭按钮
├── 主预览区：像真实 HTML 报告纸面
└── 操作区：浏览器打开 / 导出 HTML / 分享版本 / 数据状态
```
